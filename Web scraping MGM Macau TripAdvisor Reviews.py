...
pathToCotaiReviews = "Z:/MGM/My work/Python/TripReviews_mgm_cotai.csv"
...
def scrapeUrls(url): 
    ...
    while nextPage:
        ...
        try:
            ...
            continue
        try:
                with open(pathToCotaiReviews, mode='a', encoding="utf-8") as trip_data:
                    for review in reviews:
                        ratingDate = review.find('div', class_='_2fxQ4TOx').text.strip()
                        #ratingDate = ratingDate.select('div > div')[0].get_text(strip=True)
                        #ratingDate=ratingDate[ratingDate.index('a'):].split('wrote a review')
...
url='https://www.tripadvisor.com/Hotel_Review-g664891-d13328161-Reviews-MGM_Cotai-Macau.html'
urls=[]
scrapeUrls(url)
